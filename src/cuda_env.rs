...
pub struct AtariStateGpu {
    ...
    // TIA timing
    pub tia_clock: u16,
    pub tia_scanline: u16,
    pub tia_wsync: u8,
    pub tia_frame_complete: u8,
    pub tia_vsync: u8,
    pub tia_vblank: u8,
...
// Safety: AtariStateGpu is repr(C) with only primitive types, all valid when zeroed
unsafe impl DeviceRepr for AtariStateGpu {}
unsafe impl ValidAsZeroBits for AtariStateGpu {}

impl AtariStateGpu {
    /// Convert a CPU-side HeadlessAtari into a GPU state struct.
    pub fn from_headless(atari: &HeadlessAtari) -> Self {
        ...
        Self {
            ...
            port_a_output: pia.port_a_output,
            port_a_ddr: pia.port_a_ddr,
            port_a_input: pia.port_a_input,
            port_b_output: pia.port_b_output,
            port_b_ddr: pia.port_b_ddr,
            port_b_input: pia.port_b_input,
            timer_value: pia.timer_value,
            timer_prescaler: pia.timer_prescaler,
            ...
            tia_clock: tia.clock,
            tia_scanline: tia.scanline,
            tia_wsync: tia.wsync as u8,
            tia_frame_complete: tia.frame_complete as u8,
            tia_vsync: tia.vsync,
            tia_vblank: tia.vblank,
            ...
            inpt4: tia.inpt4 as u8,
            inpt5: tia.inpt5 as u8,

            paddle0: tia.paddle0,
            paddle1: tia.paddle1,
            paddle_counter: tia.paddle_counter,
            paddle_dumped: tia.paddle_dumped as u8,
            _pad4: 0,
            ...
            grp0: tia.grp0,
            grp1: tia.grp1,
            grp0_old: tia.grp0_old,
            grp1_old: tia.grp1_old,
            enam0: tia.enam0 as u8,
            enam1: tia.enam1 as u8,
            enabl: tia.enabl as u8,
            enabl_old: tia.enabl_old as u8,
            vdelp0: tia.vdelp0 as u8,
            vdelp1: tia.vdelp1 as u8,
            vdelbl: tia.vdelbl as u8,
            resmp0: tia.resmp0 as u8,
            resmp1: tia.resmp1 as u8,
            nusiz0: tia.nusiz0,
            nusiz1: tia.nusiz1,
            ...
            colubk: tia.colubk,
            ctrlpf: tia.ctrlpf,
            refp0: tia.refp0 as u8,
            refp1: tia.refp1 as u8,
            pf0: tia.pf0,
            pf1: tia.pf1,
            pf2: tia.pf2,
...
impl BatchAtariGpu {
    /// Create a new batch GPU environment with N instances.
    pub fn new(rom: Vec<u8>, n: usize) -> Result<Self, DriverError> {
        ...
        // Upload ROM to device global memory
    ...
    /// Step all instances with the given actions. Returns N*128 bytes of observations.
    pub fn step(&mut self, actions: &[u8]) -> Result<Vec<u8>, DriverError> {
        ...
        let func = self.dev.get_func("atari", "atari_frame_kernel")
            .expect("atari_frame_kernel not found");
        ...
        let obs = self.dev.dtoh_sync_copy(&self.d_obs)?;
        Ok(obs)
    }

    /// Download full GPU state for verification.
